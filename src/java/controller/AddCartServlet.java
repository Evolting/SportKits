...
public class AddCartServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (acc == null) {
            ...
            String id = request.getParameter("kitID");
            String size = request.getParameter("size");
            int quantity = Integer.parseInt(request.getParameter("quantity"));

            // TAO ITEM VOI CAC THONG TIN DA CO
            KitDAO kdb = new KitDAO();
            Kit k = kdb.getKitByID(id);
            DecimalFormat df = new DecimalFormat("#.00");
            float price = Float.parseFloat(df.format(k.getPrice()));
            Item t = new Item(k, size, quantity, (float) price);
